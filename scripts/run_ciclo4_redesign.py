...
def compute_expanded_features(df_m5):
    """
    Computes all original 5 features plus expanded features causally without lookahead:
    ...
    - Higher timeframe trend: H1 EMA50/EMA200 direction & distance (resampled causally)
    ...
    df['ATR_pct_rank'] = df['ATR'].rolling(window=1000).rank(pct=True).fillna(0.5)
    ...
    # H1 indicators (Resampled causally, shifted by 1 completed H1 bar)
    ...
    df_h1['H1_ATR14'] = calc_atr(df_h1, 14)

    df_h1_shifted = df_h1[['EMA_Fast', 'EMA_Slow', 'H1_ATR14']].shift(1)
    df = pd.merge_asof(df, df_h1_shifted, left_index=True, right_index=True, direction='backward')

    df['H1_dist_EMA50'] = ((df['Close'] - df['EMA_Fast']) / df['ATR'].replace(0, np.nan)).fillna(0.0)
    df['H1_dist_EMA200'] = ((df['Close'] - df['EMA_Slow']) / df['ATR'].replace(0, np.nan)).fillna(0.0)
    df['H1_dir_EMA50'] = np.sign(df['Close'] - df['EMA_Fast']).fillna(0.0)
    df['H1_dir_EMA200'] = np.sign(df['Close'] - df['EMA_Slow']).fillna(0.0)
    df['H1_EMA50_vs_EMA200'] = ((df['EMA_Fast'] - df['EMA_Slow']) / df['ATR'].replace(0, np.nan)).fillna(0.0)
...
def main():
    ...
    out_dir = "backtests/ciclo4_redesign"
    os.makedirs(out_dir, exist_ok=True)

    print("=== Ciclo 4: Rediseño del Motor de Señal ===")
    print("Cargando dataset M5 y calculando features ampliadas...")
    df_m5 = pd.read_parquet(parquet_path)
    df = compute_expanded_features(df_m5)
    ...
    # -------------------------------------------------------------------------
    print("\n-------------------------------------------------------------------------")
    print(" PARTE 1 — Diagnóstico de Contenido Informativo (In-Sample)")
    print("-------------------------------------------------------------------------")

    df['Target_K1'] = (df['Close'].shift(-1) > df['Close']).astype(int)
    df_clean = df.iloc[1000:-1].copy() # Warmup for rolling features and valid target

    is_mask = df_clean.index < df.iloc[is_split_idx].name
    df_is = df_clean[is_mask].copy()
    df_oos = df_clean[~is_mask].copy()

    print(f"Total barras analizadas: {len(df_clean)}")
    ...
    df_ic_auc.to_csv(os.path.join(out_dir, "ic_auc_diagnosis_is.csv"), index=False)
    print(f"Diagnóstico IC/AUC guardado en {out_dir}/ic_auc_diagnosis_is.csv")

    print("\nTop 10 Features por |IC| para horizon K=1:")
    print(df_ic_auc[df_ic_auc['Horizon_K']==1].sort_values(by='Spearman_IC', key=abs, ascending=False).head(10).to_string(index=False))

    # -------------------------------------------------------------------------
    # PARTE 2 — Ajuste de Modelo de Regresión Logística L2 en IS
    # -------------------------------------------------------------------------
    print("\n-------------------------------------------------------------------------")
    print(" PARTE 2 — Modelo Ajustado con Regularización L2")
    print("-------------------------------------------------------------------------")

    # Split IS into Train_IS (70%) and Val_IS (30%) for internal tuning
    n_is = len(df_is)
    ...
    print(f"Mejor parametro C seleccionado en Val_IS: {best_c} (Val AUC: {best_val_auc:.5f})")
    ...
    print(f"Modelo Final - AUC IS:  {is_auc:.5f}")
    print(f"Modelo Final - AUC OOS: {oos_auc:.5f}")
    ...
    df_coef.to_csv(os.path.join(out_dir, "feature_coefficients.csv"), index=False)
    print(f"Coeficientes del modelo guardados en {out_dir}/feature_coefficients.csv")
    ...
    df_preds.to_csv(os.path.join(out_dir, "model_predictions_is_oos.csv"), index=False)
    print(f"Predicciones del modelo guardadas en {out_dir}/model_predictions_is_oos.csv")

    # -------------------------------------------------------------------------
    # Backtests de Señal Aislada (Lote fijo 0.03, sin compounding, sin layering)
    # -------------------------------------------------------------------------
    print("\n-------------------------------------------------------------------------")
    print(" Evaluando Backtests del Modelo Ajustado (IS vs OOS)...")
    print("-------------------------------------------------------------------------")
    ...
    cols_order = ['Threshold', 'Set', 'trades', 'net_profit', 'net_profit_pct', 'profit_factor', 'win_rate', 'max_dd_pct', 'max_dd_usd', 'sharpe_ratio', 'ontester_score']
    df_bt_metrics = df_bt_metrics[cols_order]
    df_bt_metrics.to_csv(os.path.join(out_dir, "backtest_metrics_is_oos.csv"), index=False)
    print(f"Métricas de backtest guardadas en {out_dir}/backtest_metrics_is_oos.csv")

    # -------------------------------------------------------------------------
    # Etapa B: Evaluación bajo los 4 Perfiles de Riesgo
    # -------------------------------------------------------------------------
    print("\n-------------------------------------------------------------------------")
    print(" Etapa B: Evaluando Modelo bajo los 4 Perfiles de Riesgo Reales...")
    print("-------------------------------------------------------------------------")
    ...
    etapa_b_list = []
    chosen_th = 0.53 # Standard probability threshold
    ...
    df_etapa_b.to_csv(os.path.join(out_dir, "etapa_b_profiles_is_oos.csv"), index=False)
    print(f"Resultados Etapa B guardados en {out_dir}/etapa_b_profiles_is_oos.csv")

    # -------------------------------------------------------------------------
    # Generar README.md
    # -------------------------------------------------------------------------
    readme_content = f"""# Ciclo 4: Rediseño del Motor de Señal
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: |
"""
    top_ic_features = df_ic_auc[df_ic_auc['Horizon_K']==1].sort_values(by='Spearman_IC', key=abs, ascending=False)
    for _, row in top_ic_features.iterrows():
        feat = row['Feature']
        ic1 = row['Spearman_IC']
        pval1 = row['IC_pvalue']
        auc1 = row['ROC_AUC']

        auc6 = df_ic_auc[(df_ic_auc['Horizon_K']==6) & (df_ic_auc['Feature']==feat)]['ROC_AUC'].values[0]
        auc12 = df_ic_auc[(df_ic_auc['Horizon_K']==12) & (df_ic_auc['Feature']==feat)]['ROC_AUC'].values[0]

        cat = "Original" if feat in ['sRSI','sCCI','sSlope','sReturn','sTrend'] else "Ampliadas"
        readme_content += f"| `{feat}` | {cat} | {ic1:.6f} | {pval1:.2e} | {auc1:.4f} | {auc6:.4f} | {auc12:.4f} |\n"
...
"""
    for _, row in df_coef.iterrows():
        readme_content += f"| `{row['Feature']}` | {row['Coefficient']:.6f} | {row['Abs_Coef']:.6f} |\n"

    readme_content += f"""
### Métrica de Discriminación del Modelo (AUC)
- **AUC In-Sample (IS):** `{is_auc:.5f}`
- **AUC Out-Of-Sample (OOS):** `{oos_auc:.5f}`
...
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
"""
    for _, row in df_bt_metrics.iterrows():
        readme_content += f"| {row['Threshold']:.2f} | {row['Set']} | {row['trades']} | ${row['net_profit']:.2f} | {row['profit_factor']:.2f} | {row['win_rate']:.2f}% | {row['max_dd_pct']:.2f}% | {row['sharpe_ratio']:.2f} | {row['ontester_score']:.2f} |\n"
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
"""
    for _, row in df_etapa_b.iterrows():
        readme_content += f"| {row['Profile_Name']} | {row['IS_Trades']} | ${row['IS_NetProfit']:.2f} | {row['IS_PF']:.2f} | {row['IS_MaxDD']:.1f}% | {row['OOS_Trades']} | ${row['OOS_NetProfit']:.2f} | {row['OOS_PF']:.2f} | {row['OOS_MaxDD']:.1f}% |\n"
...
"""
    ...
    print(f"README.md generado exitosamente en {out_dir}/README.md")
    print("\n=== Ciclo 4 completado exitosamente ===")
