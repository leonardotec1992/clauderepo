"""
Tick-by-tick backtester for Strategy 3: MOMENTUM (ADX + DI) on H1 timeframe.

Data source: Dukascopy XAUUSD tick files (.csv.gz) in datos/ticks_dukascopy/
Execution rules:
- Signal evaluated on H1 candle completion:
  - BUY if ADX(14) > 25 AND +DI(14) > -DI(14) AND H1 close > open (bullish candle).
  - SELL if ADX(14) > 25 AND -DI(14) > +DI(14) AND H1 close < open (bearish candle).
...
from src.indicators import compute_atr, compute_adx_di
...
def build_h1_candles_from_ticks(tick_files: List[str]) -> pd.DataFrame:
    ...
    and computes ADX(14), +DI, -DI, and ATR(14) on H1 candles.
    """
    h1_bars_list = []
    ...
    full_h1['adx14'], full_h1['plus_di14'], full_h1['minus_di14'] = compute_adx_di(full_h1, 14)

    is_bullish = full_h1['close'] > full_h1['open']
    is_bearish = full_h1['close'] < full_h1['open']

    buy_cond = (full_h1['adx14'] > 25) & (full_h1['plus_di14'] > full_h1['minus_di14']) & is_bullish
    sell_cond = (full_h1['adx14'] > 25) & (full_h1['minus_di14'] > full_h1['plus_di14']) & is_bearish
...
def run_momentum_tick_backtest(tick_files: List[str], h1_df: pd.DataFrame, initial_capital: float = 10000.0) -> Dict[str, Any]:
    """
    Pass 2: Fast vectorized tick-by-tick backtester for MOMENTUM H1.
    """
    # Key: hour timestamp when trade can execute (next hour boundary) -> dict(signal, atr14)
    h1_signals = {}
    ...
    strat_label = "MOMENTUM (ADX+DI) H1 Ticks"

    for fpath in tick_files:
        print(f"Streaming ticks for MOMENTUM H1 simulation: {os.path.basename(fpath)}...")
        chunks = pd.read_csv(fpath, chunksize=500000)
...
def main():
    ...
    res = run_momentum_tick_backtest(tick_files, h1_df)

    output_dir = 'resultados'
    os.makedirs(output_dir, exist_ok=True)

    trades_csv_path = os.path.join(output_dir, 'trades_MOMENTUM_H1_ticks.csv')
    res['trades_df'].to_csv(trades_csv_path, index=False)
    print(f"Saved trades to {trades_csv_path}")

    plt.figure(figsize=(12, 6))
    eq_series = res['equity_series']
    plt.plot(eq_series.index, eq_series.values, label='MOMENTUM (ADX+DI) H1', color='green', linewidth=1.5)
    plt.axhline(10000.0, color='gray', linestyle='--', alpha=0.7, label='Initial Capital ($10,000)')
    plt.title('MOMENTUM-H1 Real Tick Backtest Equity Curve (XAUUSD Dukascopy)', fontsize=14, fontweight='bold')
    ...
    equity_png_path = os.path.join(output_dir, 'equity_MOMENTUM_H1_ticks.png')
